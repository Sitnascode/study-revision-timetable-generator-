(() => {
  ...
  const loadSubjects = () => {
    try { subjects = JSON.parse(localStorage.getItem(STORAGE_KEY)) || []; }
    catch (e) { subjects = []; }
  };
  ...
  const showError = msg => {
    ...
    setTimeout(() => { if (refs.formError.textContent === msg) refs.formError.textContent = ""; }, 3000);
  };
  ...
  refs.addBtn.addEventListener("click", () => {
    ...
    if (subjects.some(s => s.name.toLowerCase() === name.toLowerCase())) return showError("Subject already exists");
  ...
  refs.modalSave.addEventListener("click", () => {
    ...
    if (subjects.some((s, i) => s.name.toLowerCase() === name.toLowerCase() && i !== editingIndex))
  ...
  // Drag & Drop (unchanged)
  let initDragDrop = () => { /* keep original drag & drop logic here */ };
  ...
  refs.sidebar.addEventListener("mouseenter", () => refs.sidebar.classList.add("expanded"));
  refs.sidebar.addEventListener("mouseleave", () => refs.sidebar.classList.remove("expanded"));
